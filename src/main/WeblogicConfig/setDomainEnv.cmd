...
set SUN_JAVA_HOME=C:\PROGRA~1\Java\jdk-1.8

set DEFAULT_SUN_JAVA_HOME=C:\PROGRA~1\Java\jdk-1.8
...
set LONG_DOMAIN_HOME=C:\Oracle\Middleware\Oracle_Home\user_projects\domains\base_domain

if "%APPCDS_CLASS_LIST%"=="" (
	set APPCDS_CLASS_LIST=%DOMAIN_HOME%\WebLogic.classlist
)

if "%APPCDS_ARCHIVE%"=="" (
	set APPCDS_ARCHIVE=%DOMAIN_HOME%\WebLogic.jsa
)
...
set debugpatchAgent=-javaagent:%WL_HOME%\server\lib\debugpatch-agent.jar
...
GOTO :CMD_LINE_DONE
	...
	if /i "%noQuotesParam%" == "generateClassList" (
		set USE_JVM_SYSTEM_LOADER=true
		set GENERATE_CLASS_LIST=true
		GOTO :EOF
	)
	if /i "%noQuotesParam%" == "useArchive" (
		set USE_JVM_SYSTEM_LOADER=true
		set USE_ARCHIVE=true
		GOTO :EOF
	)
	if /i "%noQuotesParam%" == "disableDebugPatches" (
		set debugpatchAgent=
		GOTO :EOF
	) else (
...
if NOT "%JAVA_VENDOR%"=="HP" (
	if "%VM_TYPE%"=="HotSpot" (
		if "%PRODUCTION_MODE%"=="" (
			set MEM_DEV_ARGS=-XX:CompileThreshold=8000
		)
	)
)
...
@REM Set server startup configuration, if available. Only called if starting
...
@REM Set final environment user overrides, if available.

if exist %DOMAIN_HOME%\bin\setUserOverridesLate.cmd (
	call "%DOMAIN_HOME%\bin\setUserOverridesLate.cmd"
)
...
@REM Properties passed in for server start only(edit for your configuration!)

set JAVA_STARTUP_PROPERTIES=%JAVA_STARTUP_PROPERTIES%

pushd %LONG_DOMAIN_HOME%

@REM Clustering support (edit for your cluster!)

echo %JAVA_OPTIONS% | findstr -c:"-Dweblogic.management.server=" && (set ADD_ADMIN_URL_TO_CLUSTER_PROP=false) || (set ADD_ADMIN_URL_TO_CLUSTER_PROP=true) 
...
if "%ADMIN_URL%"=="" (
	...
	if "%ADD_ADMIN_URL_TO_CLUSTER_PROP%"=="false" (
		set CLUSTER_PROPERTIES=
	) else (
...
if "%local_debug%"=="true" (
	...
	set JAVA_OPTIONS=%JAVA_OPTIONS% %enableHotswapFlag% %debugpatchAgent% -ea -da:com.bea... -da:javelin... -da:weblogic... -ea:com.bea.wli... -ea:com.bea.broker... -ea:com.bea.sbconsole...
) else (
	set JAVA_OPTIONS=%JAVA_OPTIONS% %enableHotswapFlag% %debugpatchAgent% -da
)

if "%stopFlag%"=="false" (
	set JAVA_PROPERTIES=%JAVA_PROPERTIES% %JAVA_STARTUP_PROPERTIES%
)
...
@REM Append -Djava.endorsed.dirs to JAVA_OPTIONS in jdk8

%JAVA_HOME%\bin\java -version  2>&1  | findstr "1\.8\.[0-9]*_"  > NUL 
set javaStatus=%ERRORLEVEL%


if "%javaStatus%"=="0" (
	set JAVA_OPTIONS=%JAVA_OPTIONS% -Djava.endorsed.dirs=%JRE_HOME%\lib\endorsed;%MW_HOME%\oracle_common\modules\endorsed;%WL_HOME%\modules\endorsed
)
...
@REM --- Set JAR Signature Check Script ---
set CHECK_JAR_SIGNATURE="%DOMAIN_HOME%\config\security\check-jar-signature.bat"
